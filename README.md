...
### 連携後にClaude Codeへ言えること

リポジトリ直下の`CLAUDE.md`に、日本語の指示とスクリプトの対応表を書いてあります。Claude Codeはこのリポジトリで作業する時に自動で読み込むため、連携さえ済ませればユーザーはコマンドを覚える必要がありません。

- 「Todo見せて」「メモを見せて」「〇〇というメモを作って」「チェックつけて」
- **「このメモ消して」→ ゴミ箱へ移動（`delete_matome_note.py`）**

「消して」は完全削除ではなくゴミ箱への移動として扱うよう`CLAUDE.md`で指示しています。完全削除する手段はAPI側に用意しておらず、アプリのゴミ箱から本人が操作するしかありません。あわせて、実行前に対象を確認すること・子メモもまとめて移動することを伝えること・鍵付きメモには触らないことをルールとして明記しています。
